...
import{AlertTriangle,CheckCircle2,FileSpreadsheet,History,RefreshCw,Trash2,UploadCloud}from'lucide-react';
...
export default function ImportPageV2({session,reload,toast}){
 const[items,setItems]=useState([]);const[running,setRunning]=useState(false);const[history,setHistory]=useState([]);const uid=session?.user?.id;
 const total=useMemo(()=>items.reduce((s,x)=>s+(x.parsed?.rows?.length||0),0),[items]);
 async function loadHistory(){if(!uid)return;const{data,error}=await supabase.from('statement_import_history').select('*').eq('user_id',uid).order('imported_at',{ascending:false}).limit(12);if(!error)setHistory(data||[])}
 ...
 async function saveHistory(item,result,status){const{error}=await supabase.from('statement_import_history').insert({user_id:uid,file_name:item.file.name,scope:item.scope,recognized_count:item.parsed.rows.length,imported_count:result.ok,duplicate_count:result.dup,failed_count:result.fail,delimiter:item.parsed.meta.delimiter||null,header_line:item.parsed.meta.headerIndex||null,status});if(error)console.warn('Falha ao salvar histórico de importação',error)}
 async function importAll(){if(!items.length)return;const valid=items.filter(x=>x.parsed.rows.length&&!x.parsed.errors.length);if(!valid.length)return toast('Nenhum arquivo válido para importar.','error');setRunning(true);let ok=0,dup=0,fail=0;
  for(const item of valid){update(item.id,{status:'importing'});let localOk=0,localDup=0,localFail=0;for(const r of item.parsed.rows){try{const raw=`${item.scope}|${r.date}|${norm(r.description)}|${Number(r.amount).toFixed(2)}`;const hash=await sha256(raw);const payload={user_id:uid,scope:item.scope,kind:r.amount>0?'income':'expense',status:'paid',description:r.description,category:classify(r.description),amount:Math.abs(r.amount),occurred_on:r.date,source_name:item.file.name,source_hash:hash};const{error}=await supabase.from('financial_transactions').insert(payload);if(error?.code==='23505'){dup++;localDup++}else if(error){fail++;localFail++}else{ok++;localOk++}}catch{fail++;localFail++}}
   const result={ok:localOk,dup:localDup,fail:localFail};const status=localFail?(localOk||localDup?'partial':'failed'):'completed';update(item.id,{status:localFail?'partial':'done',result});await saveHistory(item,result,status);
  }
  setRunning(false);await reload(true);await loadHistory();toast(`${ok} lançamentos importados, ${dup} duplicidades ignoradas${fail?`, ${fail} falhas`:''}.`,fail?'error':'success');
 }
 ...
 <article className="mn-card"style={{marginTop:18}}><small>HISTÓRICO</small><h2 style={{display:'flex',alignItems:'center',gap:8}}><History size={19}/>Últimos extratos importados</h2>{history.length?<div style={{display:'flex',flexDirection:'column',gap:10}}>{history.map(h=><div key={h.id}style={{border:'1px solid #dce6e0',borderRadius:12,padding:12,display:'grid',gridTemplateColumns:'1fr auto',gap:8}}><div><b>{h.file_name}</b><small style={{display:'block',marginTop:4}}>{h.scope} · {new Date(h.imported_at).toLocaleString('pt-BR')} · {h.recognized_count} reconhecidos</small></div><div style={{textAlign:'right'}}><b style={{color:h.status==='completed'?'#0d8a55':h.status==='failed'?'#b42318':'#b7791f'}}>{h.imported_count} importados</b><small style={{display:'block',marginTop:4}}>{h.duplicate_count} duplicados · {h.failed_count} falhas</small></div></div>)}</div>:<div style={{padding:'20px 0',color:'#71837a'}}>Nenhum histórico de importação registrado ainda.</div>}</article>
 </section>
}
