...
function sampleData() {
  ...
  const bills = [
    { id: uid(), desc: "DAS Simples Nacional", amount: 76.9, dueDate: due.toISOString().slice(0, 10), type: "pagar", recurring: true, paid: false, wallet: "PJ" },
    { id: uid(), desc: "Aluguel", amount: 1400, dueDate: due2.toISOString().slice(0, 10), type: "pagar", recurring: true, paid: false, wallet: "PF" },
    { id: uid(), desc: "Honorários cliente Omega", amount: 890, dueDate: due2.toISOString().slice(0, 10), type: "receber", recurring: true, paid: false, wallet: "PJ" },
  ];
  return { tx, bills };
}
...
function buildMonthStats(tx, mKey, catOf) {
  ...
  const inn = mTx.filter((t) => t.amount > 0).reduce((s, t) => s + t.amount, 0);
  const out = mTx.filter((t) => t.amount < 0).reduce((s, t) => s - t.amount, 0);
  const byWallet = {};
...
export default function MetonFinanceira() {
  ...
  const metrics = useMemo(() => {
    ...
    const monthTx = filtered.filter((t) => monthKey(t.date) === nowKey);
    ...
    const evolution = keys.map((k) => {
      const mt = filtered.filter((t) => monthKey(t.date) === k);
      return {
        mes: monthLabel(k),
  ...
  const markBillPaid = (b) => {
    ...
    const t = {
      ...
      desc: b.desc, wallet: b.wallet, category: null, by: currentUser?.name,
    };
    setTx((prev) => [t, ...prev]);
    setBills((prev) => prev.map((x) => {
      ...
      return { ...x, paid: true, lastPayment: { txId } };
    }));
  ...
  const undoPayment = (b) => {
    ...
    setBills((prev) => prev.map((x) => {
      if (x.id !== b.id) return x;
      if (x.recurring) return { ...x, dueDate: lp?.prevDueDate || x.dueDate, lastPayment: null };
      return { ...x, paid: false, lastPayment: null };
    }));
  ...
  const wipeAll = () => {
    if (!window.confirm("Apagar TODOS os dados financeiros? Esta ação não pode ser desfeita.")) return;
  ...
  return (
    <div className="min-h-screen text-stone-900 pb-24" style={{ background: NUDE }}>
              ...
              <Card className="divide-y divide-stone-100">
                {filtered.slice(0, 120).map((t) => (
                  <div key={t.id} className="p-3.5 flex items-start gap-3">
                    <div className="mt-0.5 w-2 h-2 rounded-full shrink-0" style={{ background: t.amount >= 0 ? LIGHT : NUDE_DEEP, outline: t.amount >= 0 ? `1px solid ${DARK}` : "none" }} />
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-stone-400 flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {t.date.split("-").reverse().join("/")} · {t.wallet}{t.by ? ` · por ${t.by.split(" ")[0]}` : ""}
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <button onClick={() => setTx((p) => p.filter((x) => x.id !== t.id))} className="text-stone-300 mt-1">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
      ...
      {showAddTx && (
        <AddTxModal onClose={() => setShowAddTx(false)} onSave={(t) => {
          const h = txHash(t);
          ...
          setTx((p) => [{ ...t, id: uid(), category: null, by: currentUser?.name }, ...p].sort((a, b) => b.date.localeCompare(a.date)));
          setShowAddTx(false); setToast(isDup ? "Lançamento duplicado adicionado (confirmado)." : "Lançamento adicionado.");
        }} />
      )}
      {showAddBill && (
        <AddBillModal onClose={() => setShowAddBill(false)} onSave={(b) => {
          setBills((p) => [...p, { ...b, id: uid(), paid: false, by: currentUser?.name }]);
          setShowAddBill(false); setToast("Conta cadastrada.");
        }} />
      )}
      {editingBill && (
        <AddBillModal initial={editingBill} onClose={() => setEditingBill(null)}
...
function AddTxModal({ onClose, onSave }) {
      ...
      <div className="space-y-3">
        ...
        <button disabled={!ok}
          onClick={() => {
            ...
            onSave({ desc: desc.trim(), amount: type === "saida" ? -v : v, date, wallet });
          }}
          className="w-full py-2.5 rounded-xl text-white font-semibold text-sm disabled:opacity-40" style={{ background: DARK }}>
          Salvar lançamento
        </button>
...
function AddBillModal({ onClose, onSave, initial }) {
  ...
  return (
    <ModalShell title={initial ? "Editar conta" : "Nova conta"} onClose={onClose}>
      <div className="space-y-3">
        ...
        <label className="flex items-center gap-2 text-sm text-stone-700 pl-1">
          <input type="checkbox" checked={recurring} onChange={(e) => setRecurring(e.target.checked)} className="rounded" />
          Repete todo mês
        </label>
        <button disabled={!ok}
          onClick={() => onSave({ desc: desc.trim(), amount: Math.abs(parseBRNumber(amount)), dueDate, type, wallet, recurring })}
          className="w-full py-2.5 rounded-xl text-white font-semibold text-sm disabled:opacity-40" style={{ background: DARK }}>
