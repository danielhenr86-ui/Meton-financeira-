...
function patchMetonStable() {
  return {
    ...
    transform(code, id) {
      ...
      next = next.replace("<div className=\"mn-decision\"onClick={()=>setPage('controller')}","<RadarExecutive tx={tx}/><div className=\"mn-decision\"onClick={()=>setPage('controller')}");
      const required=["TransactionsV2","CardsPageV2","LoansPage","AuditPage","RemindersPage","ReportsV2","ReminderWatcher","ImportPageV2","RadarExecutive","<RadarExecutive tx={tx}/>","const ctrl=calcController"];
